        ...
        try:
            ...
            if not release_element:
                ...
                version_links = [
                    ...
                    if "/releases/tag/" in link.get("href", "") and "v" in link.text
                ]
                if version_links:
            ...
            return {
                "error": f"Failed to retrieve Pangolin version: {str(e)}, then {str(nested_e)}"
            }


def check_pangolin_data_version():
    """Check the latest version of Pangolin-data from official GitHub repo"""
    # Try the GitHub API first (most reliable)
    try:
        url = "https://api.github.com/repos/cov-lineages/pangolin-data/releases/latest"
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
        }
        response = requests.get(url, headers=headers)
        ...
        # If API fails, try parsing the HTML of the releases page
        try:
            url = "https://github.com/cov-lineages/pangolin-data/releases"
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36"
            }
            response = requests.get(url, headers=headers)
            ...
            if not release_element:
                # If still not found, try the tags page
                url = "https://github.com/cov-lineages/pangolin-data/tags"
                response = requests.get(url, headers=headers)
                response.raise_for_status()
                soup = BeautifulSoup(response.text, "html.parser")
                ...
                if tag_element:
                    ...
                    if tag_link:
                        ...
                        return {
                            "version": version,
                            "published_date": published_date,
                            "url": f"https://github.com/cov-lineages/pangolin-data/releases/tag/{version}",
                        }
                ...
                return {"error": "Could not find Pangolin-data version information"}
            ...
            return {
                "error": f"Failed to retrieve Pangolin-data version: {str(e)}, then {str(nested_e)}"
            }
...
def check_artic_sarscov2_primers():
    ...
    try:
        ...
        if not versions:
            versions = [
                row.find("a", class_="js-navigation-open").text.strip()
                for row in rows
                if row.find("svg", class_="octicon-file-directory-fill")
        ...
        versions.sort(
            key=lambda x: [
                float(n) if n.replace(".", "", 1).isdigit() else 0
...
def check_artic_ncov2019_primers():
    ...
    try:
        ...
        for a_tag in soup.find_all("a"):
            ...
            if (
                a_tag.has_attr("href")
                and "/tree/master/nCoV-2019/" in a_tag["href"]
                and a_tag.text.strip()
            ):
        ...
        if not versions:
            return {
                "error": "Could not detect ARTIC primer versions from the GitHub page"
            }
        ...
        return {"all_versions": versions, "url": url}
    except Exception as e:
...
def main():
    ...
    # Check Pangolin-data
    print("\n2. Pangolin-data Repository")
    print("-" * 60)
    pangolin_data_info = check_pangolin_data_version()
    if "error" in pangolin_data_info:
        print(f"  {pangolin_data_info['error']}")
    else:
        print(f"  Latest version: {pangolin_data_info['version']}")
        print(f"  Published date: {pangolin_data_info['published_date']}")
        print(f"  URL: {pangolin_data_info['url']}")

    # Check ARTIC
    print("\n3. ARTIC Field Bioinformatics")
    print("-" * 60)
    ...
    print("\n4. ARTIC nCoV-2019 Primer Schemes")
    print("-" * 60)
